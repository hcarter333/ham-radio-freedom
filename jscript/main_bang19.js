...
function startAnimationClick() {
	...
	function hit_fb(fb_link){
	    if(target_id != ''){  
		FB.ui({
	          method: 'send',
	          to: target_id,
	          name: 'Hit by Cannon',
	          description: 'Target your friends in an online cannon game',
	          link: fb_link,
	          });
	    }
		//FB.ui(
	...
	function bang_hit(lat, lng){
        ...
        var hittarget = false;
        var splat_scale = 2;
        if((tlat < lat+(splat_size*splat_scale)) && (tlat > lat-(splat_size*splat_scale)) && (tlng < lng+(splat_size*splat_scale)) && 
           (tlng > lng-(splat_size*splat_scale))){
        	hittarget = true;
        }
        var hitres_html = '';
        var b_html = '';
...
function set_map_target(location, id, name){
	 geocoder = new GClientGeocoder();

	 geocoder.getLatLng(location, setpoint_test_map);
	 target_id = id;
	 target_name = name;
	 target_location = location;
...
function setpoint_test_map(latlng){
	 //alert('Entered setpoint');
	friend_locs[friend_count] = latlng; 
	if(latlng == null){
	   //alert('address not found');
	   //TODO Add list of friends with no location
	 }
	 else{
